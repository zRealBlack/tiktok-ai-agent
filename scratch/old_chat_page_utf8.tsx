...
const EMOJIS = ["≡ƒÿé","Γ¥ñ∩╕Å","≡ƒöÑ","≡ƒæÅ","≡ƒÿ«","≡ƒÿó","≡ƒñö","≡ƒÆ»","≡ƒÜÇ","Γ£à","≡ƒæì","≡ƒÄë","≡ƒÆ¬","≡ƒÿì","≡ƒÖÅ","ΓÜí","≡ƒÄ»","≡ƒÆí","≡ƒñú","≡ƒÿÄ"];
...
// ΓöÇΓöÇΓöÇ Avatar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Static mock conversations ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
const INITIAL_CONVERSATIONS: Conversation[] = [
  {
    ...
    isAI: true,
    lastMessage: "╪º┘ä╪ú┘è╪¼┘å╪¬ ╪┤╪║╪º┘ä! ╪Ñ┘è┘ç ╪º┘ä┘ä┘è ╪¬╪¡╪¬╪º╪¼┘ç╪ƒ",
    time: "Now",
    unread: 0,
    online: true,
...
// ΓöÇΓöÇΓöÇ Sarie AI hook ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function useSarieChat() {
  ...
  useEffect(() => {
    ...
    if (messages.length === 0 && account?.username) {
      setMessages([{
        role: "assistant",
        content: `╪º┘ä╪ú┘è╪¼┘å╪¬ ╪┤╪║╪º┘ä! ╪╣┘å╪»┘è ┘â┘ä ╪º┘ä╪¿┘è╪º┘å╪º╪¬ ╪¿╪¬╪º╪╣╪⌐ ${account.username} (${(account.followers || 0).toLocaleString()} ┘à╪¬╪º╪¿╪╣). ╪Ñ┘è┘ç ╪º┘ä┘ä┘è ╪¬╪¡╪¬╪º╪¼┘ç╪ƒ`,
        ts: now(),
      }]);
    }
  ...
  const send = useCallback(async (text: string) => {
    ...
    try {
      ...
      setMessages(p => {
        ...
        // Save to KV (cloud ΓÇö no localStorage)
        if (currentUser?.id) {
      ...
      setMessages(p => {
        ...
        return [...u, { role: "assistant", content: "ΓÜá∩╕Å Sorry, the AI model is experiencing extremely high demand right now. Please try again in a few seconds.", ts: now() }];
      });
    } finally {
      setStreaming(false);
    }
...
function ChatPageInner() {
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 800 * 1024 && !isImage) {
      alert("ΓÜá∩╕Å File is too large! Since you don't have a storage bucket set up yet, please upload files under 800KB.");
      e.target.value = "";
      return;
    }
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row gap-4 flex-1 min-h-0 overflow-hidden pb-4 pr-4 pl-4 md:pl-0 box-border">
      ...
      {/* ΓöÇΓöÇ LEFT: Conversation List ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ CENTER: Chat Window ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        ...
        <div className="flex items-center gap-3 p-4 border-b border-[var(--glass-border)] shrink-0">
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: activeConvo.online ? "#22c55e" : "var(--text-muted)" }}>
              {activeConvo.online ? "ΓùÅ Online" : "ΓùÅ Offline"}
            </div>
          </div>
        ...
        <div style={{ padding: "14px 20px", borderTop: "1px solid var(--glass-border)", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, background: "var(--glass-elevated)", border: "1px solid var(--glass-elevated-border)", borderRadius: 24, padding: "8px 12px", minHeight: 52 }}>
            ...
            <textarea value={input} onChange={e => {
              ...
              placeholder={isAI ? "╪º╪│╪ú┘ä ╪│╪º╪▒┘è..." : "Write a message..."}
              rows={1}
      ...
      {/* ΓöÇΓöÇ RIGHT: Context Panel ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
            ...
            {activeConvo.isAI ? (
              <>
                ...
                <div style={{ marginTop: 8, padding: "10px", borderRadius: 12, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)" }}>
                  ...
                  {["╪Ñ┘è┘ç ╪ú╪╢╪╣┘ü ┘å┘é╪╖╪⌐ ╪╣┘å╪»┘è╪ƒ", "╪╡┘ä╪¡ ╪ú╪│┘ê╪ú ┘ü┘è╪»┘è┘ê", "╪¬╪¡┘ä┘è┘ä ╪º┘ä┘à┘å╪º┘ü╪│┘è┘å"].map(p => (
                    <button
                      key={p}
