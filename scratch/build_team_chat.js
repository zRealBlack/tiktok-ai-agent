...
const oldChat = fs.readFileSync('scratch/old_chat_page_utf8.tsx', 'utf8');
const teamChatHtml = fs.readFileSync('app/team-chat/[id]/page.tsx', 'utf8'); // It's currently static JSX

// We will copy the full logic of ChatPageInner, but hardcode `activeId` to the route parameter `params.id`.
// Wait, actually, just putting the entire `ChatPageInner` into `app/team-chat/[id]/page.tsx` and mapping the Team Chat UI is safer.
...
function AvatarCircle({ name, src, size = 40, online }) {
...
export default function TeamChatPage({ params }: { params: { id: string } }) {
  ...
  const confirmForward = (targetIds) => {
                ...
                >
                  ...
                  {m.reactions && m.reactions.length > 0 && (
                    <div className={\`flex gap-1 mt-1 \${isUser ? 'mr-4' : 'ml-8'} flex-wrap\`}>
                      {m.reactions.map((r, ri) => <span key={ri} onClick={() => addReaction(i, r)} className="text-xs cursor-pointer bg-white rounded-full px-2 py-0.5 border border-gray-100 shadow-sm">{r}</span>)}
                    </div>
                  )}
...
fs.writeFileSync('app/team-chat/[id]/page.tsx', newTeamChat);
console.log("Wrote fully functional team chat logic");
